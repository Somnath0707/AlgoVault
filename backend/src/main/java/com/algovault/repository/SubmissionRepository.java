...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    ...
    interface ProblemAttemptProjection {
        Long getProblemId();
        Double getProblemRating();
        String getVerdict();
        LocalDateTime getSubmittedAt();
    }

    @Query("select s.problem.id as problemId, s.problem.actualRating as problemRating, " +
           "s.verdict as verdict, s.submittedAt as submittedAt " +
           "from Submission s where s.user.id = :userId and s.problem.actualRating is not null")
    List<ProblemAttemptProjection> findAttemptProjectionsByUserId(Long userId);
    ...
    Set<String> findLeetcodeSubmissionIdsByUserIdAndLeetcodeSubmissionIdIn(Long userId, Collection<String> ids);
